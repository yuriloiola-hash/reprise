...
export function MedicoCupons({ medicoId }: MedicoCuponsProps) {
  ...
  if (loading) return <div className="p-8 text-center animate-pulse text-brand-text-muted">Carregando cupons...</div>;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between px-1">
        <h3 className="font-display font-semibold text-sm text-brand-text-muted uppercase tracking-wider flex items-center gap-2">
          <Ticket size={16} /> Gestão de Cupons
        </h3>
        <button 
        ...
        >
          {showAdd ? 'CANCELAR' : <><Plus size={12} /> NOVO CUPOM</>}
        </button>
      </div>
        ...
        <form onSubmit={handleAdd} className="bg-white p-6 rounded-[20px] border-2 border-brand-primary/10 shadow-lg space-y-5 animate-in slide-in-from-top-2 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-1.5">
              <label className="text-[10px] font-brand font-bold text-brand-text-muted uppercase tracking-widest px-1">Código (Numérico)</label>
              <input 
      ...
      {cupons.length === 0 ? (
        <div className="text-center py-10 bg-white rounded-[16px] border border-dashed border-brand-border">
          <p className="text-brand-text-muted font-sans text-sm">Nenhum cupom registrado para este médico.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3">
          {cupons.map((cupom) => (
            <div key={cupom.id} className="bg-white p-4 rounded-2xl border border-brand-border shadow-sm flex items-center justify-between group">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                  cupom.status_entrega === 'entregue' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                }`}>
                  {cupom.status_entrega === 'entregue' ? <Check size={20} /> : <Clock size={20} />}
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-sm font-bold text-brand-text tracking-tight">{cupom.codigo}</span>
                    <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded ${
                      cupom.produto === 'Patz Cp' ? 'bg-blue-50 text-blue-700' : 'bg-indigo-50 text-indigo-700'
                    }`}>
                      {cupom.produto}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 mt-0.5">
                    <span className="text-[10px] text-brand-text-muted font-medium flex items-center gap-1">
                      {cupom.tipo_envio === 'presencial' ? <Handshake size={10} /> : <Smartphone size={10} />}
                      {cupom.tipo_envio}
                    </span>
                    <span className="text-[10px] text-brand-text-muted font-mono">
                      Prometido em: {cupom.data_prometida ? new Date(cupom.data_prometida).toLocaleDateString('pt-BR') : '—'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {cupom.status_entrega === 'entregue' ? 'ENTREGUE' : 'MARCAR ENTREGUE'}
                </button>
                <button 
